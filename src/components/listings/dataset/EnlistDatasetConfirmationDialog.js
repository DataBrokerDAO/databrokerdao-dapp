...
class EnlistConfirmationDialog extends Component {
  ...
  finishStep(step) {
    if (step === STEP_INTRO) {
      if (!this.props.token) this.setState({ stepIndex: STEP_REGISTRATION });
        ...
        const stakeDTX = BigNumber(
          parseInt(this.props.dataset.stake, 10)
        ).times(BigNumber(10).pow(18));
        if (BigNumber(this.props.balance).isGreaterThan(stakeDTX)) {
          this.props.enlistDataset(this.props.dataset);
      ...
      this.props.history.push(`/listings#tab-datasets`);
  ...
  render() {
    const loading = this.props.enlistingDataset;
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p>
            To enlist your dataset you need DTX tokens. As DataBroker DAO is
            currently in beta, we will provide you with free demo tokens.
          </p>
        </div>
        <div
          style={{
        ...
        <div
          style={{
            display: this.state.stepIndex === STEP_ENLISTING ? 'block' : 'none'
          }}
        >
        ...
        >
          ...
          <p>From now on, people are able to purchase your stream data.</p>
        </div>
        <div
          style={{
            display:
...
const mapStateToProps = state => ({
  ...
  enlistingDataset: state.listings.enlistingDataset,
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    enlistDataset: dataset => dispatch(LISTING_ACTIONS.enlistDataset(dataset)),
