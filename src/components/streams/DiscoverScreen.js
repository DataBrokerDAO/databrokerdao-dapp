            ...
            {error => {
              return (
                <DiscoverMap
                  googleMapURL={
                    !error.message
                      ? `https://maps.googleapis.com/maps/api/js?v=3.33&libraries=places&key=${APIKey}`
                      : `https://maps.google.cn/maps/api/js?v=3.33&libraries=places&key=${APIKey}`
                  }
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    setMapCenterByAddress: address =>
      dispatch(STREAMS_ACTIONS.setCenter({ address }))
  };
}
